...
const Block2StateRoads: React.FC<Block2StateRoadsProps> = () => {
  ...
  const handleBaseYearChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === '') {
      ...
      dispatch(setStateRoadBaseYear(new Date().getFullYear()));
      return;
    }
    const year = parseInt(value);
    if (!isNaN(year)) {
      // Дозволяємо будь-яке введення чисел, браузер сам контролює min/max
      dispatch(setStateRoadBaseYear(year));
    }
  }, [dispatch]);
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="sm:col-span-2">
                <Label htmlFor="stateRoadBaseRate">
                  Встановлений норматив річних фінансових витрат на ЕУ 1 км дороги II кат. державного значення в цінах <span className="bg-yellow-200 px-1 rounded font-semibold">{stateRoadBaseYear}</span> року (тис. грн/км)
                </Label>
                <div className="grid gap-4 sm:grid-cols-2 mt-2">
                  <Input
                    ...
                    placeholder="наприклад: 604.761"
                  />
                  <div className="flex items-center gap-2">
                    <Label htmlFor="stateRoadBaseYear" className="whitespace-nowrap text-sm">
                      Рік нормативу:
                    </Label>
                    <Input
